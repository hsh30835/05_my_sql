...
VALUES
(4,'user03','pass03','고길동','남','010-2322-2323');
...
-- 아래의 경우 null을 값으로 인식하지 않기 때문에 nuique 제약조건을 위배하지 않는다.
...
DROP TABLE IF EXISTS user_forignkey1;
CREATE TABLE IF NOT EXISTS user_forignkey1(
	...
	email varchar(255),
	grade_code int,
	-- foreign key와 references는 세트
	-- grade_code를 user_grade를 참조할꺼다?
...
SELECT * FROM user_forignkey1;

DESCRIBE user_forignkey1;
...
(user_no, user_id, user_pwd, user_name, gender, phone, email, grade_code)
VALUES
(3, 'user01', 'pass01', '유관순', '홍길동', '010-1234-5678', 'hong123@gmail.com', 50);
...
DROP TABLE IF EXISTS user_forignkey2;
CREATE TABLE IF NOT EXISTS user_forignkey2(
	user_no int PRIMARY key,
	user_id varchar(255) NOT NULL,
	user_pwd varchar(255) NOT NULL,
	user_name varchar(255) NOT NULL,
...
INSERT INTO user_forignkey2
(user_no, user_id, user_pwd, user_name, gender, phone, email, grade_code)
VALUES
(1, 'user01', 'pass01', '홍길동', '남', '010-1234-5678', 'hong123@gmail.com', 10),
(2, 'user02', 'pass02', '유관순', '여', '010-777-7777', 'yu77@gmail.com', 20);

SELECT * FROM user_forignkey2;

-- 부모 테이블의 grade_code 수정
DROP TABLE IF EXISTS user_forignkey1;
-- on delete set null 옵션으로 인해 부모의 값을 수정하면 자식의 값은 null이 된다.
UPDATE user_grade
   SET grade_code = NULL
 WHERE grade_code = 10;
...
SELECT * FROM user_forignkey2; -- 자식
...
VALUES
(NULL,'안중근','남',27);
-- age의 check 제약조건을 확인하면 age>=19로 설정이 되어 있기 때문에 오류가 발생되는 것이다.
INSERT INTO user_check
VALUES
(NULL,'유관순','여',17);
 ...
 * 컬럼 타입이 date일 시 cuurent_date만 가능
 * 컬럼 타입이 dateTime일 시 currunet_time과 current_rimestamp, now() 모두 가능
 * */
...
INSERT INTO tbl_country -- DEFAULT 생락해도된다
(country_code)
VALUES(NULL);

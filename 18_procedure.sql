 ...
 * 일련의 sql 문이나 비즈니스 로직을 포함하는 데이터베이스의 객체이다.
 * 주로 반복적으로 사용되는 쿼리나 로직을 하나로 묶어서 재사용할 수 있도록 하는데 사용한다.
 * 
 * 특징
 * 1. 이름 : 프로시저는 고유한 이름을 가져야 하며 이름을 통해 프로시저를 호출하거나 참조할 수 있다.
 * 2. 매개변수 : 프로시저는 매개변수를 받을 수 있으며 이를 통해 외부에서 값을 전달받아 프로시저내에서 구형이 가능하다
